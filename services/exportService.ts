...
import type { Core, Section, Microfossil, Folder, DataPoint } from '../types';
...
const generateSectionContent = (doc: jsPDFType, section: Section, microfossils: Microfossil[], userEmail: string, sectionTitle: string) => {
    ...
    addSectionBlock('Section & Recovery Details', () => {
        addTable([], [
            ...
            ['Section Depth', section.sectionDepth ? `${section.sectionDepth} mbsf` : 'N/A'],
            ['Sample Interval', section.sampleInterval ? `${section.sampleInterval} m` : 'N/A'],
        ]);
    });
...
export const exportFolderToOdv = (folder: Folder, cores: Core[], allSections: Section[], options: { variables: string[], excludeFlaggedData: boolean }) => {
    ...
    const dynamicHeaders = options.variables.map(key => ODV_PROXY_LABELS[key] || key);
    ...
    odvContent += `// Generated on: ${new Date().toISOString()}\n`;
    odvContent += `//---\n`;
    ...
    sectionsToExport.forEach(section => {
        ...
        const dataPoints = options.excludeFlaggedData
            ? section.dataPoints.filter(dp => dp.qcFlag === 0 || dp.qcFlag === undefined)
            : section.dataPoints;

        dataPoints.forEach(dp => {
            const staticValues = [
                core.project.replace(/\s/g, '_'),
                core.id.replace(/\s/g, '_'),
                'C', // Core
                core.location.lon,
                core.location.lat,
                core.waterDepth,
            ...
            // Only include row if at least one dynamic value is present
